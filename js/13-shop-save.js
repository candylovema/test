...
async function cloudSaveGame(silent = false) {
    ...
    try {
        if (!silent) logSys('正在上傳存檔至雲端 Google 試算表...');
        
        // 1. 標準化面向參照，避免循環依賴
        normalizeFacingRefsForSave();
        
        // 2. 暫時移除 summonsV2 避免垃圾資料入檔
        let _sv2 = player.summonsV2;
        if (_sv2 && _sv2.length) player.summonsV2 = [];
        ...
        try {
            ...
            savePayload = JSON.stringify({ v: SAVE_VERSION, p: player, ms: mapState, ticks: state.ticks });
        } finally {
            if (_sv2 && _sv2.length) player.summonsV2 = _sv2;
        }
        ...
        if (resJson && resJson.status === 'success') {
            if (!silent) logSys('<span class="text-emerald-300 font-bold">☁️ 雲端同步完成！</span>');
            return true;
        } else {
            throw new Error((resJson && resJson.message) || 'API 儲存失敗');
        }
    } catch (e) {
        console.error('[cloudSaveGame] failed', e);
        if (!silent) logSys('<span class="text-red-400 font-bold">⚠ 雲端同步失敗：' + e.message + '</span>');
        return false;
    }
}
...
async function cloudLoadGame() {
    ...
    try {
        ...
        if (resJson && resJson.status === 'success' && resJson.data) {
            ...
            let _u = _saveUnwrap(signedSave);
            if (_u.signed && !_u.ok) {
                alert('下載的雲端存檔完整性校驗未通過，可能已被修改。');
                return false;
            }
            
            // 寫入本地存檔
            _lzSetStoredRaw(key, signedSave);
            logSys('<span class="text-emerald-300 font-bold">☁️ 雲端存檔下載成功，正在套用進度...</span>');
            
            // 重載遊戲
            loadGame();
            return true;
        } else {
            throw new Error((resJson && resJson.message) || '未在雲端找到該欄位的存檔');
        }
    } catch (e) {
        console.error('[cloudLoadGame] failed', e);
        logSys('<span class="text-red-400 font-bold">⚠ 自雲端下載存檔失敗：' + e.message + '</span>');
        return false;
    }
}
